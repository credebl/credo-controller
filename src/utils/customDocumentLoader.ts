...
function isW3CDeprecatedCredeblSchema(url: string, agentContext: AgentContext): boolean {
    agentContext.config.logger.debug(`Checking if w3c url(${url}) contains deprecated domain for agent: ${agentContext.config.label}`)
    ...
    return async function (url: string): Promise<DocumentLoaderResult> {
        try {
            ...
            if (isW3CDeprecatedCredeblSchema(url, agentContext)) {
                agentContext.config.logger.debug(`Found w3c url(${url}) containing deprecated domain for agent: ${agentContext.config.label}`)
